...
bool RecipicalOverlap(RP_READ & first, RP_READ & second) {
	...
	if (diff(first.PosA, first.PosA1) > distance) return false;
	if (diff(first.PosB, first.PosB1) > distance) return false;
	if (diff(second.PosA, second.PosA1) > distance) return false;
	if (diff(second.PosB, second.PosB1) > distance) return false;
    float cutoff = 0.9;
...
void ModifyRP(std::vector <RP_READ> & Reads_RP) {
	...
	for (unsigned first = 0; first < Reads_RP.size(); first++) {
		...
		if (Reads_RP[first].ChrNameA == Reads_RP[first].ChrNameB && diff(Reads_RP[first].PosA, Reads_RP[first].PosB) < 500) Reads_RP[first].Visited = true;
